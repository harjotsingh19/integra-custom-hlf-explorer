...
class CRUDService {
	...
	getTransactionDetail(txHash) {
		const sqlTxById = ` select t.id, t.blockid, t.txhash,t.validation_code,t.payload_proposal_hash,t.creator_msp_id,t.endorser_msp_id,t.chaincodename,t.type,t.createdt,t.read_set,
				t.write_set,channel.name as channelName from TRANSACTIONS as t inner join channel on t.channel_genesis_hash=channel.channel_genesis_hash and t.network_name=channel.network_name
	...
	async getBlockActivityPage(network_name, channel_genesis_hash, page, size) {
		const totalRecordBlockSql = `select count(*) from blocks`;
		const totalRecord = await this.sql.getRowsBySQlQuery(totalRecordBlockSql);
		...
		const sqlBlockActivityList = `select blocks.blocknum,blocks.txcount ,blocks.datahash ,blocks.blockhash ,blocks.prehash,blocks.createdt, (
			SELECT  array_agg(txhash) as txhash FROM transactions where blockid = blocks.blocknum and
				channel_genesis_hash = '${channel_genesis_hash}' and network_name = '${network_name}' group by transactions.blockid ),
																																							channel.name as channelname  from blocks inner join channel on blocks.channel_genesis_hash = channel.channel_genesis_hash  where
																																	blocks.channel_genesis_hash ='${channel_genesis_hash}' and blocknum >= 0 and blocks.network_name = '${network_name}'
																																order by blocks.blocknum desc limit ${size} offset ${(page - 1) * size}`;
		const row = await this.sql.getRowsBySQlQuery(sqlBlockActivityList);
	...
	getBlockDetail(blockDataHash) {
		const blockDetailSql = `select blocks.blocknum,blocks.txcount ,blocks.datahash ,blocks.blockhash ,blocks.prehash,blocks.createdt, (
			SELECT  array_agg(txhash) as txhash FROM transactions where blockid = blocks.blocknum group by transactions.blockid ),
																																							channel.name as channelname  from blocks inner join channel on blocks.channel_genesis_hash = channel.channel_genesis_hash  where
																																	blocks.datahash ='${blockDataHash}' and blocknum >= 0`;
		return this.sql.getRowsBySQlQuery(blockDetailSql);
	}
	 ...
	 * @memberof CRUDService
	 */
	getTxList(network_name, channel_genesis_hash, blockNum, txid, from, to, orgs, isFromSet = false) {
		let byOrgs = false;
		if (orgs && orgs !== '') {
			byOrgs = true;
		}
		...
		const sqlTxListByOrgs = ` select t.blockid, t.id, t.creator_msp_id,t.txhash,t.type,t.chaincodename,t.createdt,channel.name as channelName from transactions as t
       inner join channel on t.channel_genesis_hash=channel.channel_genesis_hash and t.network_name = channel.network_name where  t.blockid >= ${blockNum} and t.id >= ${txid} and t.creator_msp_id in (${orgs}) and
		...
		const sqlTxList = ` select t.blockid, t.id, t.creator_msp_id,t.txhash,t.type,t.chaincodename,t.createdt,channel.name as channelName from transactions as t
       inner join channel on t.channel_genesis_hash=channel.channel_genesis_hash and t.network_name = channel.network_name where  t.blockid >= ${blockNum} and t.id >= ${txid} and
		...
		const openSqlTxList = ` select t.blockid, t.id, t.creator_msp_id,t.txhash,t.type,t.chaincodename,t.createdt,channel.name as channelName from transactions as t
		inner join channel on t.channel_genesis_hash=channel.channel_genesis_hash and t.network_name = channel.network_name where  t.blockid >= ${blockNum} and t.id >= ${txid} and
								t.channel_genesis_hash = '${channel_genesis_hash}' and t.network_name = '${network_name}' order by  t.id desc`;
		...
		if(!isFromSet) {
			return this.sql.getRowsBySQlQuery(openSqlTxList);
		}
	...
	getBlockAndTxList(
      ...
      select (select c.name from channel c where c.channel_genesis_hash =
		...
		const openSqlBlockTxList = `select a.* from  (
			select (select c.name from channel c where c.channel_genesis_hash =
			   '${channel_genesis_hash}' and c.network_name = '${network_name}') as channelname, blocks.blocknum,blocks.txcount ,blocks.datahash ,blocks.blockhash ,blocks.prehash,blocks.createdt, blocks.blksize, (
			  SELECT  array_agg(txhash) as txhash FROM transactions where blockid = blocks.blocknum and
			   channel_genesis_hash = '${channel_genesis_hash}' and network_name = '${network_name}') from blocks where
			   blocks.channel_genesis_hash ='${channel_genesis_hash}' and blocks.network_name = '${network_name}' and blocknum >= 0 order by blocks.blocknum desc)  a where  a.txhash IS NOT NULL`;
		...
		if(!isFromSet) { 
		    console.log(openSqlBlockTxList);
			return this.sql.getRowsBySQlQuery(openSqlBlockTxList);
		}
