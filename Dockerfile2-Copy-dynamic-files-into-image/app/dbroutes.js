...
const dbroutes = (router, platform) => {
	...
	router.get('/blockActivity/:channel_genesis_hash/:page/:size', (req, res) => {
		...
		if (channel_genesis_hash && !isNaN(page) && !isNaN(size)) {
			dbCrudService
				.getBlockActivityPage(req.network, channel_genesis_hash, page, size)
				.then(row => {
					if (row) {
						return res.send(row);
					}
	...
	router.get('/txByDay/:channel_genesis_hash/:days', (req, res) => {
		...
		if (channel_genesis_hash && !isNaN(days)) {
			dbStatusMetrics
				.getTxByDay(req.network, channel_genesis_hash, days)
				.then(rows => {
					if (rows) {
						return res.send({ status: 200, rows });
					}
	...
	router.get('/txByMonth/:channel_genesis_hash/:months', (req, res) => {
		...
		if (channel_genesis_hash && !isNaN(months)) {
			dbStatusMetrics
				.getTxByMonth(req.network, channel_genesis_hash, months)
				.then(rows => {
					if (rows) {
						return res.send({ status: 200, rows });
					}
	...
	router.get('/getAllTx/:channel_genesis_hash', (req, res) => {
		...
		if (channel_genesis_hash) {
			dbStatusMetrics
				.getAllTransactions(req.network, channel_genesis_hash)
				.then(rows => {
					if (rows) {
						return res.send({ status: 200, rows });
					}
	...
	router.get('/blocksByDay/:channel_genesis_hash/:days', (req, res) => {
		...
		if (channel_genesis_hash && !isNaN(days)) {
			dbStatusMetrics
				.getBlocksByDay(req.network, channel_genesis_hash, days)
				.then(rows => {
					if (rows) {
						return res.send({ status: 200, rows });
					}
	...
	router.get('/blocksByMonth/:channel_genesis_hash/:months', (req, res) => {
		...
		if (channel_genesis_hash && !isNaN(months)) {
			dbStatusMetrics
				.getBlocksByMonth(req.network, channel_genesis_hash, months)
				.then(rows => {
					if (rows) {
						return res.send({ status: 200, rows });
					}
	...
	router.get('/getAllBlocks/:channel_genesis_hash', (req, res) => {
		...
		if (channel_genesis_hash) {
			dbStatusMetrics
				.getAllBlocks(req.network, channel_genesis_hash)
				.then(rows => {
					if (rows) {
						return res.send({ status: 200, rows });
					}
	...
	router.get('/tokenhistoryforowner/:integraid',
		async (req, res) => {
			http.get(`${sdkConfig.url}/tokenhistoryforowner/${req.params.integraid}`,  (resP) => {
	...
	router.get('/holders',
		async (req, res) => {
				// Do stuff with response				
				http.get(`${sdkConfig.url}/holders?limit=${req.query.limit}`,  (resP) => {
				resP.setEncoding("utf8");
				let body = "";
				resP.on("data", data => {
				  body += data;
