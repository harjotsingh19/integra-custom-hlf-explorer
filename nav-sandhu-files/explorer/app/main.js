...
const cors_whitelist_domains_1 = require("./cors-whitelist-domains");
/**
 *
 *
 * @class Broadcaster
...
function startExplorer() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let whitelist = [ 
            ...
            'http://10.0.1.4:8080', 
            'http://localhost:8080', 
            'http://172.174.188.41:80',
            'http://localhost:3000',
            'http://localhost:3011',
            'http://localhost:3005',
            'https://www.mainnet.integraledger.com', 
            'https://mainnet.integraledger.com', 
            'https://testnet.integraledger.com',
            'http://testnet.integraledger.com',
            'https://explorer.integraledger.com',
            'http://explorer.integraledger.com', 
            'http://www.explorer.integraledger.com'
        ];
        
        // if(cors_whitelist_domains_1.config.domains.length) {
        //     whitelist = cors_whitelist_domains_1.config.domains;
        // }

        // logger.error(" cors_whitelist_domains_1 ",cors_whitelist_domains_1.config);
        ...
        let dmn = cors_whitelist_domains_1.config.maindomain;
        if(!dmn) {
            dmn = '*.integraledger.com';
        }
        
        explorer.getApp().use((req, res, next) => {
            res.set({
                "Content-Security-Policy": `default-src * 'self' ${dmn}; connect-src * ${dmn}; img-src * 'self' data: https: http:; script-src 'self' 'unsafe-inline' 'unsafe-eval' *; style-src 'self' 'unsafe-inline' *`,
                "Access-Control-Allow-Origin": `${dmn}`,
                "Access-Control-Allow-Headers":"*",                
            });            
            next();
        });
        ...
        //use cors middleware
        explorer.getApp().use(cors_1.default(corsOptions));
        //enable pre-flight
        explorer.getApp().options('*', cors_1.default(corsOptions));
        // ============= start server =======================
        server.listen(port, () => {
