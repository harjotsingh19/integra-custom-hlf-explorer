...
export class MetricService {
	...
	async getAllBlocks(network_name, channel_genesis_hash) {
		const sqlStartTime = 'select createdt from blocks order by createdt ASC limit 1';	
		const rows = await this.sql.getRowsBySQlQuery(sqlStartTime,[]);	
		const now :any = new Date();	
		const startTimeDate :any = new Date(rows[0].createdt);	
		const days:any = Math.round((now - startTimeDate) / (1000 * 60 * 60 * 24));	
	...
	async getAllTransactions(network_name, channel_genesis_hash) {
