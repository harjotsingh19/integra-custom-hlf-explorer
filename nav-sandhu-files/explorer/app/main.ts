...
async function startExplorer() {
	...
	let whitelist = ['http://localhost:8080', 'http://localhost:3000', 'https://www.mainnet.integraledger.com', 'https://mainnet.integraledger.com', 'https://testnet.integraledger.com', 
	'https://explorer.integraledger.com', 
	'http://explorer.integraledger.com', 'http://www.explorer.integraledger.com']
	...
	explorer.getApp().use((req, res, next) => {		
	    res.set({        
        	"Content-Security-Policy": "default-src * 'self' *.integraledger.com; connect-src * *.integraledger.com; img-src * 'self' data: https: http:; script-src 'self' 'unsafe-inline' 'unsafe-eval' *; style-src 'self' 'unsafe-inline' *"
	    })
	    next();
    });
	...
	//use cors middleware
	explorer.getApp().use(cors(corsOptions));		
			
	//enable pre-flight
	explorer.getApp().options('*', cors(corsOptions));
