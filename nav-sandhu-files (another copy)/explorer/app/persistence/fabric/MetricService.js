...
class MetricService {
    ...
    getChaincodeCount(network_name, channel_genesis_hash) {
        return this.sql.getRowsBySQlCase('select count(1) c from chaincodes where channel_genesis_hash=$1 and network_name=$2 ', [channel_genesis_hash, network_name]);
    }
    /**
     *
     *
    ...
    getBlocksByDay(network_name, channel_genesis_hash, days) {
        const sqlPerDay = `  with days as (
          ...
          select
            ...
            count(createdt)
          from days
          left join BLOCKS on date_trunc('day', BLOCKS.createdt) =days.datetime and channel_genesis_hash=$1 and network_name=$2
          group by 1
          order by 1 `;
        return this.sql.getRowsBySQlQuery(sqlPerDay, [
            channel_genesis_hash,
    ...
    getAllBlocks(network_name, channel_genesis_hash) {
        return __awaiter(this, void 0, void 0, function* () {
            const sqlStartTime = 'select createdt from blocks order by createdt ASC limit 1';
            const rows = yield this.sql.getRowsBySQlQuery(sqlStartTime, []);
            const now = new Date();
            const startTimeDate = new Date(rows[0].createdt);
            const days = Math.round((now - startTimeDate) / (1000 * 60 * 60 * 24));
            return this.getBlocksByDay(network_name, channel_genesis_hash, days);
        });
    }
    /**
     *
    ...
    getAllTransactions(network_name, channel_genesis_hash) {
        return __awaiter(this, void 0, void 0, function* () {
            const sqlStartTime = 'select createdt from transactions order by createdt ASC limit 1';
            const rows = yield this.sql.getRowsBySQlQuery(sqlStartTime, []);
            const now = new Date();
            const startTimeDate = new Date(rows[0].createdt);
            const days = Math.round((now - startTimeDate) / (1000 * 60 * 60 * 24));
            return this.getTxByDay(network_name, channel_genesis_hash, days);
        });
    }
}
