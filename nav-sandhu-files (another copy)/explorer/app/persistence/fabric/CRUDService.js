...
class CRUDService {
    ...
    getBlockActivityPage(network_name, channel_genesis_hash, page, size) {
        return __awaiter(this, void 0, void 0, function* () {
            const totalRecordBlockSql = `select count(*) from blocks`;
            const totalRecord = yield this.sql.getRowsBySQlQuery(totalRecordBlockSql, []);
            const sqlBlockActivityList = `select blocks.blocknum,blocks.txcount ,blocks.datahash ,blocks.blockhash ,blocks.prehash,blocks.createdt, (	
			SELECT  array_agg(txhash) as txhash FROM transactions where blockid = blocks.blocknum and	
            ...
            const row = yield this.sql.getRowsBySQlQuery(sqlBlockActivityList, []);
            return {
                status: 200,
                row,
     ...
     * @memberof CRUDService
     */
    getTxList(network_name, channel_genesis_hash, blockNum, txid, from, to, orgs, isFromSet) {
     ...
     * @memberof CRUDService
     */
    getBlockAndTxList(network_name, channel_genesis_hash, blockNum, from, to, orgs, isFromSet) {
     ...
     * @memberof CRUDService
     */
    saveTransaction(network_name, transaction) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isValidRow(c)) {
                ...
                yield this.sql.updateBySql('update chaincodes set txcount =txcount+1 where channel_genesis_hash=$1 and network_name = $2 and name=$3', [transaction.channel_genesis_hash, network_name, transaction.chaincodename]);
